...
geometry_msgs::Point ConversionBridge::rosPoint32ToPoint(geometry_msgs::Point32 p32) {
    geometry_msgs::Point p;
    p.x = static_cast<double>(p32.x);  // 转换为 double
    p.y = static_cast<double>(p32.y);
    p.z = static_cast<double>(p32.z);
    return p;
}
std::vector<geometry_msgs::Point> ConversionBridge::rosPoint32ToPointMulti(std::vector<geometry_msgs::Point32> p32s) {
